...
`main` へマージされた変更を GitHub Actions が自動でデプロイする。
手作業でのデプロイを続ける場合、本手順は不要である。

人手による運用では「コードを直したがデプロイを忘れる」「デプロイしたが
実行時エラーで動いていないことに気づかない」が実際に発生した。
本ワークフローは型検査とビルド検証を通したうえでデプロイし、
**デプロイ後に `/health` が `ok` を返すことを確認する**。確認が通らなければ失敗する。

### 6-1. Cloudflare の API トークンを作成する

Cloudflare ダッシュボードの **My Profile → API Tokens → Create Token** を開き、
`Edit Cloudflare Workers` のテンプレートから作成する。
必要な権限は Workers Scripts の Edit のみである。

あわせて **Account ID** を控える。ダッシュボードの Workers & Pages の画面に
表示されている。

### 6-2. リポジトリのシークレットへ登録する

GitHub のリポジトリで **Settings → Secrets and variables → Actions** を開き、
**New repository secret** から以下の 2 件を登録する。

| 名称 | 値 |
| --- | --- |
| `CLOUDFLARE_API_TOKEN` | 手順 6-1 で作成したトークン |
| `CLOUDFLARE_ACCOUNT_ID` | Cloudflare の Account ID |

シークレットはリポジトリが公開設定であっても第三者からは参照できない。
トークンをチャットや課題管理システムへ貼らず、この画面から直接入力すること。

### 6-3. 動作確認

`src/` 配下を変更して `main` へマージするか、GitHub の **Actions** タブから
`Deploy` を選び **Run workflow** を実行する。

**完了の判定**: ワークフローが成功し、`デプロイ先の疎通確認` のステップに
`疎通確認に成功しました` と表示されること。

### 6-4. ワークフローが動く条件

| 契機 | 実行される内容 |
| --- | --- |
| `main` への push（`src/` / `wrangler.jsonc` / `package.json` などの変更時） | 型検査 → ビルド検証 → デプロイ → 疎通確認 |
| pull request | 型検査 → ビルド検証まで（デプロイは行わない） |
| Actions タブからの手動実行 | 型検査 → ビルド検証 → デプロイ → 疎通確認 |

ドキュメントのみの変更ではデプロイされない。不要な実行を避けるため、
対象を `src/` などに限定している。

> **注意**: 公開リポジトリでは Actions のログを誰でも閲覧できる。
> `wrangler deploy` はデプロイ先の URL を出力するため、本ワークフローでは
> 成功時にその出力をログへ出さず、URL をマスクしている。
> ただしデプロイが失敗した場合は調査のため出力を表示するので、
> その際は URL がログに現れる。
...
要求は成功するため、応答本文を読むまで気づけない。
化け方によって原因が異なるため、次の表で切り分ける。

| 症状 | 原因 | 対処 |
| --- | --- | --- |
| `??????` のように `?` が並ぶ | 送信時のエンコーディング。`-Body` へ文字列をそのまま渡している | `[System.Text.Encoding]::UTF8.GetBytes()` でバイト列へ変換して渡す |
| `縺` `繝` `繧` などの漢字が並ぶ | スクリプトファイルの文字コード。BOM の無い UTF-8 を PowerShell 5.1 が CP932 として読んでいる | ファイルを UTF-8 (BOM 付き) で保存し直す |

いずれの場合も、文字化けした値はそのままデータベースへ保存されている。
当該の行は Supabase の Table Editor から削除し、登録をやり直すこと。
...
`5.1` の場合、本書で扱う問題（`curl` の別名、要求本文のエンコーディング、
スクリプトファイルの文字コード）がいずれも発生する。
**PowerShell 7 以降ではすべて発生しない**ため、導入できるのであれば
そちらを推奨する。`winget install Microsoft.PowerShell` で導入でき、
`pwsh` で起動する。

### スクリプトファイルの文字コード

コマンドを `.ps1` ファイルへ保存して実行する場合、
**Windows PowerShell 5.1 は BOM の無い UTF-8 ファイルを CP932 として読む。**
日本語を含むスクリプトは、実行前の読み込み時点で文字化けする。
`縺` `繝` `繧` といった文字が並ぶのがこの症状の特徴である。

対処は次のいずれかとする。

- ファイルを **UTF-8 (BOM 付き)** で保存する
  （VS Code であれば画面右下の文字コード表示から `Save with Encoding` を選ぶ）
- PowerShell 7 (`pwsh`) を用いる（BOM の無い UTF-8 を正しく読む）
- スクリプトへ日本語を直接書かず、対話的に実行する
